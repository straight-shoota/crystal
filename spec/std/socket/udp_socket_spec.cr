...
describe UDPSocket, tags: "network" do
  ...
  each_ip_family do |family, address, unspecified_address|
    ...
    it "sends and receives messages" do
      ...
      bytes_read, client_addr = server.receive(buffer.as_unsafe_slice)
      message = String.new(buffer.as_unsafe_slice[0, bytes_read])
      ...
      {% unless flag?(:win32) %}
        bytes_read, client_addr = server.receive(buffer.as_unsafe_slice[0, 4])
        message = String.new(buffer.as_unsafe_slice[0, bytes_read])
