...
struct UUID
  ...
  def self.new(slice : Slice(UInt8), variant : Variant? = nil, version : Version? = nil)
    ...
    slice.copy_to(bytes.as_unsafe_slice)
  ...
  def self.random(random : Random = Random::Secure, variant : Variant = :rfc4122, version : Version = :v4) : self
    ...
    random.random_bytes(new_bytes.as_unsafe_slice)
  ...
  def self.v1(*, clock_seq : UInt16? = nil, node_id : MAC? = nil) : self
    ...
    IO::ByteFormat::BigEndian.encode(time_low, uuid.as_unsafe_slice[0..3])
    IO::ByteFormat::BigEndian.encode(time_mid, uuid.as_unsafe_slice[4..5])
    IO::ByteFormat::BigEndian.encode(time_hi, uuid.as_unsafe_slice[6..7])
    IO::ByteFormat::BigEndian.encode(seq, uuid.as_unsafe_slice[8..9])

    if node_id
      6.times do |i|
        uuid.as_unsafe_slice[10 + i] = node_id[i]
      end
    else
      Random::Secure.random_bytes(uuid.as_unsafe_slice[10..15])
  ...
  def self.v2(domain : Domain, id : UInt32, node_id : MAC? = nil) : self
    ...
    uuid[9] = domain.to_u8
    IO::ByteFormat::BigEndian.encode(id, uuid.as_unsafe_slice[0..3])
  ...
  def self.v3(name : String, namespace : UUID) : self
    ...
    hash = klass.digest do |ctx|
      ctx.update namespace.bytes.as_unsafe_slice
      ctx.update name
    end
  ...
  def to_s(io : IO) : Nil
    ...
    io.write_string(buffer.as_unsafe_slice)
  end

  def hexstring : String
    @bytes.as_unsafe_slice.hexstring
  end
