...
class Crystal::Iocp::EventLoop < Crystal::EventLoop
  ...
  def run(blocking : Bool) : Bool
    ...
    if next_event.wake_at > now
      ...
      timed_out = Iocp::EventLoop.wait_queued_completions(wait_time, alertable: blocking) do |fiber|
  ...
  def connect(socket : ::Socket, addr : ::Socket::Addrinfo | ::Socket::Address, timeout : ::Time::Span?) : IO::Error?
    if socket.type.stream?
      connect_stream(socket, addr, timeout)
    else
      connect_connectionless(socket.fd, addr, timeout)
    end
  end
  ...
  def schedule_overlapped(timeout : Time::Span?, line = __LINE__) : Bool
    ...
    Crystal::EventLoop.current.enqueue(timeout_event)

    Crystal::Scheduler.reschedule

    Crystal::EventLoop.current.dequeue(timeout_event)
  end
