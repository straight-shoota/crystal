...
abstract class IO
  ...
  def skip(bytes_count : Int) : Nil
    ...
    while bytes_count > 0
      read_count = read(buffer.as_unsafe_slice[0, Math.min(bytes_count, buffer.size)])
  ...
  def skip_to_end : Nil
    buffer = uninitialized UInt8[DEFAULT_BUFFER_SIZE]
    while read(buffer.as_unsafe_slice) > 0
    end
  end
  ...
  def self.copy(src, dst) : Int64
    buffer = uninitialized UInt8[DEFAULT_BUFFER_SIZE]
    count = 0_i64
    while (len = src.read(buffer.as_unsafe_slice).to_i32) > 0
      dst.write buffer.as_unsafe_slice[0, len]
      count &+= len
    end
    count
  end
  ...
  def self.copy(src, dst, limit : Int) : Int64
    ...
    buffer = uninitialized UInt8[DEFAULT_BUFFER_SIZE]
    remaining = limit
    while (len = src.read(buffer.as_unsafe_slice[0, Math.min(buffer.size, Math.max(remaining, 0))])) > 0
      dst.write buffer.as_unsafe_slice[0, len]
      remaining &-= len
    end
    limit - remaining
  ...
  def self.same_content?(stream1 : IO, stream2 : IO) : Bool
    ...
    while true
      read1 = stream1.read(buf1.as_unsafe_slice)
      if read1.zero?
        # First stream is EOF, check if the second has more.
        return stream2.read_byte.nil?
      end
      read2 = stream2.read_fully?(buf2.as_unsafe_slice[0, read1])
