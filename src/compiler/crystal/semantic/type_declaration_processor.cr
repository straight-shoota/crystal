...
struct Crystal::TypeDeclarationProcessor
  ...
  private def raise_not_initialized_in_all_initialize(node : ASTNode, name, owner)
    raise SemanticError.new "instance variable '#{name}' of #{owner} was not initialized directly in all of the 'initialize' methods, rendering it nilable. Indirect initialization is not supported.", node
  end
