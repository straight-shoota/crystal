...
class HTTP::WebSocket::Protocol
  ...
  private def write_payload(data)
    ...
    @io.write mask_array.as_unsafe_slice
  ...
  private def write_masked_data(data, mask_array)
    ...
    until remaining_data.empty?
      ...
      @io.write(masked_data.as_unsafe_slice[0, chunk_size])
  ...
  private def read_header
    ...
    @io.read_fully(@header.as_unsafe_slice)
    ...
    # Read mask, if needed
    if masked?
      @io.read_fully(@mask.as_unsafe_slice)
      @mask_offset = 0
    end
  ...
  def self.new(host : String, path : String, port = nil, tls : HTTP::Client::TLSContext = nil, headers = HTTP::Headers.new)
    ...
    begin
      ...
      random_key = Base64.strict_encode(StaticArray(UInt8, 16).new { rand(256).to_u8 }.as_unsafe_slice)
  ...
  def self.key_challenge(key)
    ...
    {% else %}
      Base64.strict_encode(OpenSSL::SHA1.hash(key + GUID).as_unsafe_slice)
    {% end %}
  end
end
